...
Python中一切皆对象，类定义完成后，会创建一个**类对象**，并在当前作用域中定义一个与类同名的名字指向该类对象。
...
模块可以是Python源码形式，也可以是经过编译的C/C++对象文件，两者用法一样。
...
当准备执行`import`语句时，Python将按顺序遍历该目录列表，并采用第一个满足`import`需求的模块。如果搜索路径中找不到合适的模块，则会引发`ImportError`异常。
...
在环境变量中设置的目录，将人插入到`sys.path`的**最前面**。
...
路径配置文件的扩展名为`.pth`，位于一些特殊目录中。只要把自定义的模块搜索位置添加到这些文件中，它们就会添加到`sys.path`的**末尾**，这样解释器就可搜索到。详见模块`site`的标准库文档。

> 在Windows中，可以将`.pth`文件放在`sys.prefix`指向的目录中。
...
```python
#可以模块中的任何对象名称，包括以下划线开头的名称
>>> from somemodule import somefunction, _anotherfunction, …

#导入somemodule模块中所有的公有对象名称（即未以下划线开头的名称），以及模块（或包的 __init__.py 文件）中存在名为 __all__ 的名称列表（不管是否以下划线开头）
...
在同一个交互式会话中，如果模块所在的文件发生了改动，`import`是不会自动加载改动内容的，并且，即使再次输入`import …`命令也不会重新加载该模块。这时，可使用模块`importlib`中的函数`reload`，它接受一个参数（要重新加载的模块），并返回重新加载的模块。重新加载模块，会再次执行模块中的代码。
