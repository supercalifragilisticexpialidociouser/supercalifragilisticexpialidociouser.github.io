...
> 环境变量`LD_LIBRARY_PATH`提供了共享链接库文件的搜索位置，搜索目录之间使用逗号分隔。注意，该环境变量不是由GCC所读取，而是由可执行文件读取，并动态链接到共享链接库。
...
使用`-p`选项会在程序中加入一些特殊的函数，以当执行程序时输出剖析信息（profiling information）。剖析信息相当有用，可以辅助解决性能相关的问题，因为它可以帮助了解程序中哪些函数正在耗费时间。剖析信息被存储到一个名为`mon.out`的文件中。然后可以使用prof工具程序来以多种方式来分析剖析信息。
...
make在执行构建时，需要一个构建文件来告诉make如何去构建程序。这个构建文件默认是`makefile`或`Makefile` 文件，也可以使用`-f`选项来显式指定一个构建文件。
...
只要依赖发生了变化，并且该依赖要么不依赖其他依赖，要么它所依赖的依赖没有发生变化，make就会执行相应的命令脚本，以构建目标。如果该依赖所依赖的依赖也发生了变化，则递归地先处理上一级依赖的变化，然后再执行本规则的命令脚本，以构建目标。
...
根据定义的方式不同，变量可以分为递归展开（recursively expanded）变量和简单展开（simply expanded）变量。在递归展开变量中，嵌套的变量在每次展开时都会重新计算。而在简单展开变量中，嵌套变量在定义时就展开。如果后续修改了嵌套变量，递归展开变量在之后的展开中会反应嵌套变量的修改，而简单展开变量不受嵌套变量的修改影响。
...
变量名称如果包含多个字符，则在命令脚本引用时，必须使用`$(变量名)`形式；如果变量名只有一个字符，则可采用`$变量名`形式。
