...
在命令中，链接库要放在源文件之后。如果有多个链接库，则被依赖的库要放在依赖它的库之后。例如：`foo.o`依赖于`bar1`库，而`bar1`库依赖于`bar2`库，则应该输入：
...
生成的可执行程序在开始执行前先要载入内存中。在宿主环境中，这个任务是由操作系统完成的。在独立环境中，程序的载入必须由手工完成，也可能是通过把可执行代码置入只读内存（ROM）来完成。

### 执行

在宿主环境中，通常有一个启动程序与程序链接在一起。它负责处理一系列日常事务，如收集命令行参数以便使程序能够访问它们。

### 终止

程序可由多种不同原因终止，正常终止由`main`函数返回或调用了`exit`，也可能由某些事件而终止，还可能出现错误而终止。在宿主环境中，启动程序将再次取得控制权，并执行各种不同的日常任务。
...
注释可以出现在空格可以出现的地方（字符串除外），预处理器会把每个注释都替换成一个空格。因此，`min/*max*/Value`将变成两个标记：“min Value”。
...
在声明变量时，也可以同时将变量初始化为指定值：

```c
double percent = 0.01, taxrate = 28.5;

// 初始化变量的值不能超过该变量类型的取值范围，C编译器可能不会捕获这种错误！
...
与变量类似，常量也是程序使用的数据存储位置。但是，储存在常量中的值在程序执行期间不可更改。
...
按照惯例，使用`#define`定义符号常量名中所有字母都大写。
...
严格来说，用`const`关键字定义的符号常量实质是只读变量。
